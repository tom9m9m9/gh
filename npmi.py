...
with open('n_kv/n_kv_{}.txt'.format(name))as f1:
    data1=np.loadtxt(f1)

with open('theta_k/theta_k_{}.txt'.format(name))as f2:
    data2=np.loadtxt(f2)

with open('phi_kv/phi_kv_{}.txt'.format(name))as f3:
    data3=np.loadtxt(f3)
